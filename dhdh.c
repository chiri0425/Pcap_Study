...
void *number(){
        scanf("%d",&enter);
       /* if(enter==1){
	printf("\nPacket_Type/Reason        Incomming     Outgoing\n");
	printf("------------------------------------------------\n");
	printf("DHCP_DISCOVER                 %d            0   \n",discover);
	printf("DHCP_OFFER                    0            %d   \n",offer);
	printf("DHCP_REQUEST                  %d            0   \n",request); 
	printf("DHCP_ACK                      0            %d   \n", ack);
	printf("DHCP_RELEASE                  %d            0   \n",release);
...
int main(void) {
  ...
  puts("Enter the interface number.");
  scanf("%d", &inum);
  ...
  if((handle= pcap_open_live(dev->name, 65536,   1,  0,  errbuf  )) == NULL) {
...
//  printf("\nlistening on %s...\n", dev->description);  
  pcap_freealldevs(alldevs);   
        ...
        puts("detects packets.\n");
	puts("If you want to check the statistics, enter 1 ");	
